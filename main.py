...
LIBRARY_GROUP_ID = int(os.environ.get("LIBRARY_GROUP_ID") or "0")  # المجموعة الوحيدة المسموح لها بالبحث
...
# كلمات وصفية لا تدخل في عبارة البحث نفسها عند البداية.
...
def is_library_group(chat) -> bool:
    """يقبل البوت الرسائل من مجموعة المكتبة المحددة فقط."""
    return bool(
        chat
        and chat.type in {"group", "supergroup"}
        and LIBRARY_GROUP_ID
        and chat.id == LIBRARY_GROUP_ID
    )


def is_library_update(update: Update) -> bool:
    return is_library_group(update.effective_chat)


async def send_generic_error(update: Update) -> None:
    """يرسل رسالة عامة فقط، مع إبقاء التفاصيل في السجل الداخلي."""
    message = update.effective_message
    if not message or not is_library_update(update):
        return
    try:
...
async def search_library(query: str, channels: list) -> list:
...
async def _do_relay(
    ...
    # تبقى رسالة الملف في مجموعة الريلاي ولا يتم حذفها.
...
def normalize_search_query(text: str) -> str:
    ...
    الموجودة في بدايتها، مع الإبقاء على عبارة البحث كما كتبها المستخدم.
    """
    cleaned = []
    for char in text:
...
async def handle_search(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_library_update(update) or not update.message or not update.message.text:
        return
    try:
        query = extract_group_query(update.message.text.strip())
        if not query:
            return
        ...
        if not data["channels"]:
            await send_generic_error(update)
            return

        msg = await update.message.reply_text(f"جاري البحث في المكتبة عن: {query}...")
        results = await search_library(query, data["channels"])
        words = query.split()
        if not results and len(words) > 2:
            results = await search_library(" ".join(words[:2]), data["channels"])
        ...
        if not results:
            await msg.edit_text(f"لم يتم العثور على نتائج مكتبية لـ: {query}")
            return

        text = (
            f"نتائج المكتبة عن: {query}\n"
        ...
        logger.exception("Unhandled library search error")
        await send_generic_error(update)


async def cb_page(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    q    = update.callback_query
    if not q or not q.message or not is_library_group(q.message.chat):
        if q:
            await q.answer()
        return
...
async def cb_send_library_item(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    q = update.callback_query
    if not q or not q.message or not is_library_group(q.message.chat):
        if q:
            await q.answer()
    ...
    try:
        ...
        success, _ = await deliver_via_relay(
            destination_chat_id,
            r["chat"],
            r["msg_id"],
            ctx.bot,
        ...
        logger.exception("Unhandled library delivery error")
        await send_generic_error(update)


async def error_handler(update: object, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """يسجل التفاصيل داخليًا ولا يعرضها للمستخدمين."""
    if ctx.error:
        logger.error(
...
# -- التشغيل --------------------------------------------------------
def register_library_handlers(app: Application) -> None:
    """يسجل وظائف المكتبة داخل Application واحد يملكه البوت الرئيسي."""
    app.add_handler(CallbackQueryHandler(cb_page, pattern="^pg:"))
    app.add_handler(CallbackQueryHandler(cb_send_library_item, pattern="^sb:"))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_search))
    app.add_error_handler(error_handler)


def build_library_application() -> Application:
...
def main() -> None:
    # التشغيل المستقل متوقف افتراضيًا حتى لا يبدأ polling ثانٍ لنفس التوكن.
    standalone = os.environ.get("RUN_LIBRARY_STANDALONE", "").strip().lower()
    if standalone not in {"1", "true", "yes"}:
        logger.info(
            "Library module mode: no polling started. "
            "Register it in the main bot Application."
        )
        return
    if not LIBRARY_GROUP_ID:
        raise RuntimeError("LIBRARY_GROUP_ID must be configured")

    app = build_library_application()
    register_library_handlers(app)
    logger.info("Library bot started in standalone mode...")
