...
def find_search_owner(search_id: str) -> Optional[int]:
    """يعثر على صاحب البحث عند الضغط على زر من مستخدم آخر."""
    session = _searches_collection.find_one(
        {"search_id": search_id},
        {"user_id": 1},
    )
    return int(session["user_id"]) if session and "user_id" in session else None
    ...
    for abs_idx, r in enumerate(results[start:end], start=start):
        buttons.append([
            InlineKeyboardButton(
                r["name"][:64],
                callback_data=f"sb:{search_id}:{abs_idx}:{owner_id}",
            )
        ])
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(
            "السابق",
            callback_data=f"pg:{search_id}:{page - 1}:{owner_id}",
        ))
    if end < len(results):
        nav.append(InlineKeyboardButton(
            "التالي",
...
async def handle_search(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await msg.edit_text(
        text,
        reply_markup=results_keyboard(results, 0, search_id, user_id),
    )
...
async def cb_page(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    parts = q.data.split(":")
    encoded_owner_id = None
    if len(parts) >= 4:
        try:
            encoded_owner_id = int(parts[3])
        except ValueError:
            encoded_owner_id = None
    if encoded_owner_id is not None and encoded_owner_id != q.from_user.id:
        await q.answer(
            "هذا البحث ليس لك، ابدأ بحثاً جديداً.",
            show_alert=True,
        )
        return
    if encoded_owner_id is None and len(parts) >= 3:
        owner_id = find_search_owner(parts[1])
        if owner_id is not None and owner_id != q.from_user.id:
            await q.answer(
                "هذا البحث ليس لك، ابدأ بحثاً جديداً.",
                show_alert=True,
            )
